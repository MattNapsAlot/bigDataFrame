.test <- function(dir=system.file("unitTests", package="bigDataFrame"), testFileRegexp = "^test_.*\\.R$") {
...
.runTestSuite <- function(dir, testFileRegexp, testFuncRegexp, suiteName) {
        ...
        suite <- defineTestSuite(name=paste("bigDataFrame RUnit Test Suite", suiteName),
                        dirs=dir,
        ...
        if (length(details <- .failure_details(result)) >0) {
                ...
                cat("\n\n")
                stop(paste(suiteName, " tests failed for package bigDataFrame"))
        }
        result
}
